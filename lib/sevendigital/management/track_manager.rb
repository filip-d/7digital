module Sevendigital
  ...
  class TrackManager < Manager
    ...
    def get_details(id, options={})
      api_response = @api_client.make_api_request("track/details", {:trackId => id}, options)
    ...
    def get_chart(options={})
      api_response = @api_client.make_api_request("track/chart", {}, options)
      @api_client.chart_item_digestor.list_from_xml(api_response.content.chart)
    end

    def build_preview_url(id, options={})
      api_request = @api_client.create_api_request("track/preview", {:trackId => id}, options)
      @api_client.operator.create_request_uri(api_request)
    end

    def search(query, options={})
      api_response = @api_client.make_api_request("track/search", {:q => query}, options)
