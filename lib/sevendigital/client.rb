module Sevendigital
  ...
  COMMON_REQUEST_PARAMETERS = [:shop_id, :country, :page, :page_size, :image_size]
    ...
    def verbose?
     very_verbose? or @verbose or @configuration.verbose
    end

    def very_verbose?
      @very_verbose or @configuration.verbose.to_s == "very_verbose"
    end
  ...
  private
    ...
    #@private
    def logger
      path = @configuration.logfile_path.blank? ? STDOUT : @configuration.logfile_path
